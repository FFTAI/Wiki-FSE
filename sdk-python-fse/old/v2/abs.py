...
def get_config_info(ip):
    ...
    for i in range(3):  # Wi-Fi 连接时，广播一次不一定能全部接收到所有设备的回复 ，需要多广播几次
        ...
        try:
            r_data, addr = udp_sorket.recvfrom(1024)  # 接收局域网中回复数据，每次接收最大1024
            print('config info {}:{}'.format(addr, r_data.decode('utf-8')))
            break
        except socket.timeout:
            print('socke timeout')
...
def get_rssi(addrList, rssi_dirt):
    ...
    for i in addrList:
        ...
        try:
            ...
            # print('udp received from {}:{}'.format(addr, r_data.decode('utf-8')))
            json_obj = json.loads(r_data.decode('utf-8'))  # 解析json数据包
            if json_obj['result']['connect_mode'] == 'wifi':
                d_rssi = json_obj['result']['RSSI']  # 获取abs角度值 ，弧度值
            else:
                d_rssi = 'eth'
            # print(d_abs)
...
#  ota指令
def OTAupdate(addrList, ota_dirt):
    data = {
        "id": 0,
        "method": "OTA.Update",
        "params": "",
    ...
    json_str = json.dumps(data)
    for i in addrList:
        udp_sorket.sendto(json_str.encode('utf-8'), (i, remote_port))  # 发送ota指令
        try:
            r_data, addr = udp_sorket.recvfrom(1024)
            print('udp received from {}:{}'.format(addr, r_data.decode('utf-8')))
        except socket.timeout:
    ...
    while True:
        for i in addrList:
            ...
            try:
                ...
                # print('udp received from {}:{}'.format(addr, r_data.decode('utf-8')))
                json_obj = json.loads(r_data.decode('utf-8'))  # 解析json数据包
                cur = json_obj['result']['cur']
                total = json_obj['result']['total']
                ota_dirt[i] = "%.1f%%" % (cur / total * 100)
                print(ota_dirt)
            except socket.timeout:
                print('timeout,send agin:', i)
...
#  重启指令
def set_config(_ip, _config_dict):
    data = {
        "id": 0,
        "method": "Set.Config",
        "params": "",
    }
    data['params'] = _config_dict
    json_str = json.dumps(data)
    for i in range(3):  # Wi-Fi 连接时，广播一次不一定能全部接收到所有设备的回复 ，需要多广播几次
        udp_sorket.sendto(json_str.encode('utf-8'), (_ip, remote_port))  # udp 广播
        try:
